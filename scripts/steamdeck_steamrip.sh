...
extract_steamrip_rar() {
    ...
    # Проверяем Proton или Wine
    if ! check_proton && ! check_wine; then
        print_warning "Proton и Wine не найдены"
        read -p "Установить Wine для запуска Windows игр? (y/n): " confirm
        if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
            print_message "Установка Wine..."
            sudo pacman -S wine --noconfirm
        else
            print_error "Proton или Wine необходимы для запуска RAR игр"
            return 1
        fi
    else
        if check_proton; then
            print_success "Proton найден - будет использоваться для запуска игр"
        elif check_wine; then
            print_success "Wine найден - будет использоваться для запуска игр"
        fi
    fi
...
create_steam_shortcut() {
    ...
    # Создаем обертку для игры с Proton (Wine как fallback)
    # Используем функцию из lib/proton_utils.sh
    if create_wrapper_script "$exe_path" "$game_dir" "$game_name"; then
        local wrapper_script="$game_dir/${game_name}_run.sh"
        print_success "Создана обертка с Proton/Wine: $wrapper_script"
    else
        print_error "Не удалось создать обертку"
        return 1
    fi
